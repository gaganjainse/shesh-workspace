...
title: Efficiency
type: how-to
summary: "This doc lists free strategies to make sessions last longer (60 min → 120-180 min) and avoid waste."
...
## Selective clone — biggest win (36M → 2M)
...
## Other free efficiency strategies
...
The `setup_worker.py` does this by default. For large repos (`shesh-voice 41M`, `shesh-desktop 22M`) Shesh uses `--filter=blob:none` which downloads only commit+tree, not blobs until needed.

### B) no Rust toolchain in arena
Rust toolchain `~/.cargo` + `~/.rustup` = ~1 GB → immediate workspace-over-budget. The `SESSION_HANDOFF.md` says do NOT install Rust in sandbox — CI has Rust. For Brain work that needs `cargo test`, use `Containerfile` (Arch container with Rust) or run tests only on component that changed, not `cargo test --workspace`.
...
```bash
# Instead of the full make check
...
The workers now have `--component` filter to run only that component's tests.
...
**GitHub Models free tier** (alternative, no money): GitHub now offers free Models API `https://models.github.ai/inference` with `GITHUB_TOKEN` (no PAT needed) —  free for public repos. The project could add `tools/llm_free.py` that calls `https://models.inference.ai.azure.com` with `GITHUB_TOKEN` — free, no OpenAI key.

You said you do everything free — the project already do. No need for OpenAI API.

### H) platform worker needs zero src clones
If you are Worker-Platform (docs, ADR, Containerfile, install.sh, CI), you does not need any `src/` clones. Just work on ecosystem repo itself. That's most efficient: 0 extra MB, 0 extra files → longest session.

The `ROLE_MAP["platform"] = []` reflects this.

### The maintainer) use GitHub actions for true hours (free for public repos)
GitHub Actions free for public repos: 2000 min/month Ubuntu. The `swarm-scheduled.yml` runs every hour, 3-5 min each, uses `GITHUB_TOKEN` not PAT, no cost.

While traveling with phone, **does not keep 5 Arena tabs** — keep 1 orchestrator + let Actions do janitor work. Actions run even if phone sleeps, for 1-2 days.
...
### J) does not clone in every chat — share via `src/` persistence
Workspace `/home/user` persists across sessions (except `.cache`, `__pycache__`, etc). If you clone `shesh-memory` once via `setup_worker.py --component shesh-memory`, next session `src/shesh-memory` still exists — no need to re-clone. The `clone_repo()` checks `if dest.exists(): git pull --ff-only --depth 1` — fast.
