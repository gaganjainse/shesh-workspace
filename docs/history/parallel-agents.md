...
type: explanation
summary: "TL;DR: Open 1 orchestrator + 2-3 workers in Arena.ai."
...
- Big project: 19 components, 40 docs, the test suite — one session can't finish open item.
...
- **Atomic claim:** `claim_issue_atomic(issue_number, agent_id)`:
  1. **Lock ref** `refs/heads/swarm/claims/issue-<N>` — POST `/git/refs` with main SHA
     ...
     - The project tested: agent-A creates lock → 201, agent-B tries same lock → 422 already claimed
...
- `tools/swarm/orchestrator.py` — seed open item → Issues or file queue, dashboard, monitor stale claims, heartbeat
