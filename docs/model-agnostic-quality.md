...
type: explanation
summary: "'role': {'type': 'string', 'enum': ['coder','researcher','vision','critic']},."
...
> **Problem you reported:** Different models have different responses and abilities, you does not know if they will work properly and complete your work. Bad experience with quality variance.

> **Goal:** Workflow is model-agnostic — you can setup omniroute and enable all free options (Ollama local, Groq free, OpenRouter :free, GitHub Models free, HuggingFace free) and get **same quality output shape**, with rigorous validation so different models does not produce different quality.
...
- `extract_json()` reliable: tries fenced block, then raw JSON, then balanced brace scan `{...}` — handles model that outputs extra text.
- `validate_against_schema()` checks required keys, not JSON parse.
...
### Fallback chain — model-agnostic routing, free-first
...
### LLM-as-judge grading + eval harness — quality consistency measured
...
Or set `LITELLM_CONFIG` env to `litellm_config.yaml` that lists free models — Shesh already provides `manifests/models.toml` which can be converted to LiteLLM config.
...
## Summary: why bad experience will not repeat
