...
title: Travel Mode
type: how-to
summary: "GitHub Actions runs on GitHub's infra (Ubuntu runners), not in your browser."
...
- **In GitHub Action:** The project could run `supervise.sh --loop` in Action, but Action has no LLM to implement code — it would loop picking open item and failing to implement. The janitor action therefore performs **non-LLM janitor tasks** (locks, docs sync, re-queue) which CAN run for hours unattended.

If you want true LLM coding for hours in GitHub Actions, you need to add an LLM API key secret (e.g., `OPENAI_API_KEY`) and a script that calls LLM to implement tasks — that's possible but not yet built. The maintainer can add `tools/llm_worker.py` that calls OpenAI/Anthropic/Ollama API to implement open item, then Action could run it hourly.
...
4. **Enable Janitor Action (true hours):**
   ...
   - It will seed Issues from open item , re-queue stale claims, push locks/docs sync
...
The maintainer can add `.github/workflows/swarm-llm-worker.yml` that:
- Runs every 2 hours via cron
- Uses secret `OPENAI_API_KEY` or `ANTHROPIC_API_KEY` (you add in repo Settings → Secrets)
- Calls `tools/llm_worker.py` (The maintainer would write) that picks one pending Issue, calls LLM API to generate patch, runs `make check`, pushes branch, opens PR
...
Do you want me to add that? If yes, tell me which LLM provider (OpenAI, Anthropic, local Ollama via tunnel) and The maintainer will implement.
...
- **Do not rely on 4-5 sidebar workers on phone** — mobile OS will throttle them, they will pause.
